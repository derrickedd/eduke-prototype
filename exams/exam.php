...
<body>
	 ...
	 <script>
		 ...
		 function moreExams(value){
			 $.ajax({
				 type:"post",
				 url:"exam-process-function.php",
				 data:"page="+value+"&action=showexamlist",
				 success:function(data){
					 $("#list-results").html(data);
				 }
			 });
		 ...
		 function deleteExam(exam_id){
			 if (confirm('Are you sure you want to delete the exam?')) {
				 $.ajax({
					 type:"post",
					 url:"exam-process-function.php",
					 data:"exam_id="+exam_id+"&action=deleteexam",
					 success:function(data){
						 location.reload();
					 }
				 });
