<?php
	 ...
	 else
	 {
		 ...
		 header('Location: ../../login');
				 ...
				 $grading_id = $classlevel_id = $score_type = $max_score = $min_score = "";
					 ...
					 if ($fail == "")
					 {
						 ...
						 $query1 = "SELECT MAX(grading_id) FROM grading";
						 $result = mysql_query($query1);
						 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
						 $row = mysql_fetch_row($result);
						 $grading_id = $row[0]+1;
						 
						 if ($score_type == 'D1') {$score_value = 1;}
						 elseif ($score_type == 'D2') {$score_value = 2;}
						 elseif ($score_type == 'C3') {$score_value = 3;}
						 elseif ($score_type == 'C4') {$score_value = 4;}
						 elseif ($score_type == 'C5') {$score_value = 5;}
						 elseif ($score_type == 'C6') {$score_value = 6;}
						 elseif ($score_type == 'P7') {$score_value = 7;}
						 elseif ($score_type == 'P8') {$score_value = 8;}
						 elseif ($score_type == 'F9') {$score_value = 9;}
							 
						 $query2 = "INSERT INTO grading VALUES('$grading_id', '$classlevel_id', '$score_type', '$score_value', '$max_score', '$min_score')";
						 queryMysql($query2);
						 ...
						 header("Refresh:1; url=gradingtype1.php?classlevel_id=$classlevel_id");
					 }
					 else{
						 echo $fail;
					 }
