<?php
	 ...
	 else
	 {
		 ...
		 header('Location: ../../login');
				 ...
				 $advancedgrading_id = $score_type = $max_score = $min_score = "";
					 ...
					 if ($fail == "")
					 {
						 ...
						 $query1 = "SELECT MAX(advancedgrading_id) FROM advancedgrading";
						 $result = mysql_query($query1);
						 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
						 $row = mysql_fetch_row($result);
						 $advancedgrading_id = $row[0]+1;
						 
						 if ($score_type == 'A') {$score_value = 6;}
						 elseif ($score_type == 'B') {$score_value = 5;}
						 elseif ($score_type == 'C') {$score_value = 4;}
						 elseif ($score_type == 'D') {$score_value = 3;}
						 elseif ($score_type == 'E') {$score_value = 2;}
						 elseif ($score_type == 'O') {$score_value = 1;}
						 elseif ($score_type == 'F') {$score_value = 0;}
							 
						 $query2 = "INSERT INTO advancedgrading VALUES('$advancedgrading_id', '$score_type', '$score_value', '$max_score', '$min_score')";
						 queryMysql($query2);
						 ...
						 echo "<!doctype html>
							 <html lang=\"en\">
							 <head>
								 <title>Eduke - Advanced Grade Added</title>
						 ...
						 <h3 style=\"padding:10px;border-bottom:solid 1px #eee;font-size:20px;\">ADVANCED GRADE ADDED</h3>
						 ...
						 header("Refresh:1; url=advancedgrading.php");
					 }
					 else{
						 echo $fail;
					 }
