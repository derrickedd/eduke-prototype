     ...
     <div id="frame">
	     ...
	     <div id="navigation">
		     ...
		     <p class="subject-nav">
				 ...
				 <span class="extra-subject-nav">
				     <a href="../subjects/addsubject.php" class="selected-nav">Add Subject</a>
	     ...
	     <div id="content-reload">
			 <div id="boxlist-content">
			     <h1>ADD SUBJECT</h1>
				 <p><b>Directory: Subjects</b> / <b>Add Subject</b></p>
				 
				 <div id="addclass">
				 <form action="subjectadded.php" id="form-content" method="POST" enctype="multipart/form-data">
					     <p>SUBJECT NAME: <br><input type="text" style="padding:5px;" name="subject_name"></p>
					     <p>NUMBER OF PAPERS: <br><input type="text" style="padding:5px;" name="number_of_papers"></p>
						 <?php
						     $query = "SELECT classlevel_id, classlevel_name FROM classlevel";
							 $result = mysql_query($query);
							 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
							 $rows = mysql_num_rows($result);
							 
							 echo "<p>CLASS LEVEL: <br><select name=\"classlevel_id\"><option value=\"NONE\">-Select Level-</option>";
							 for ($j = 0 ; $j < $rows ; ++$j)
							 {
								 $row = mysql_fetch_row($result);
								 echo "<tr>";
								 for ($k = 0 ; $k < 1 ; ++$k)
									 $classlevel_id = $row[0];
									 $classlevel_name = $row[1];
									 echo "<option value=$classlevel_id>$classlevel_name</option>";
								 echo "</tr>";
							 }
							 echo "</select></p>";
						 ?>
						  <?php
						     $query = "SELECT class_category_id, class_category FROM classcategory";
							 $result = mysql_query($query);
							 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
							 $rows = mysql_num_rows($result);
							 
							 echo "<p>CLASS CATEGORY: <br><select name=\"class_category_id\"><option value=\"NONE\">-Select Category</option>";
							 for ($j = 0 ; $j < $rows ; ++$j)
							 {
								 $row = mysql_fetch_row($result);
								 echo "<tr>";
								 for ($k = 0 ; $k < 1 ; ++$k)
									 $class_category_id = $row[0];
									 $class_category = $row[1];
									 echo "<option value=$class_category_id]>$class_category</option>";
								 echo "</tr>";
							 }
							 echo "</select></p>";
						 ?>
						 <p>DESCRIPTION: <br><input type="text" style="padding:5px;" name="description"></p>
					     <p><input type="submit" style="background-color:green;color:#fff;cursor:pointer;border:solid 1px green!important;border-radius:3px;" 
						 value="Add Subject"></p>
				 </form>
				 </div>
			 </div>
