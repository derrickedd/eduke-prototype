...
<body>
     <div id="frame">
	     ...
	     <div id="content-reload">
			 ...
			 <?php
					 $class_id = $class_alias = $teacher_id = $classlevel_id = $class_category_id = "";

					 if(isset($_POST['class_alias'])){
						 $class_alias = sanitizeString($_POST['class_alias']);
					     $class_alias = string_to_uppecase($class_alias);}
					 ...
					 if (isset($_POST['class_category_id']))
						 $class_category_id = sanitizeString($_POST['class_category_id']);
					 
					 $fail = validate_class_alias($class_alias);
					 $fail .= validate_teacher_id($teacher_id);
					 $fail .= validate_classlevel_id($classlevel_id);
					 $fail .= validate_class_category_id($class_category_id);
					 ...
					 if ($fail == "")
					 {
						 ...
						 $query1 = "SELECT MAX(class_id) FROM schoolclass";
						 $result = mysql_query($query1);
						 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
						 $row = mysql_fetch_row($result);
						 $class_id = $row[0]+1;

						 $query2 = "INSERT INTO schoolclass VALUES('$class_id', '$class_alias', '$teacher_id', '$classlevel_id', '$class_category_id')";
						 queryMysql($query2);
						 
				         echo "<br><br><br><br><br><br><br>
						 <div style=\"background-color:#fff;width:620px;\">
						     <h3>CLASS ADDED</h3>
							 <p>The class has been added</p>
						 </div>";
     ...
     function validate_class_alias($field)
	 {
         if ($field == "") return "No Class name was entered<br />";
         return "";
     }
	 
	 function validate_teacher_id($field)
	 {
         if ($field == "") return "No teacher was entered<br />";
		 if ($field == "NONE") return "No Teacher was selected<br />";
         return "";
     }
	 
	 function validate_classlevel_id($field)
	 {
         if ($field == "") return "No class level id was entered<br />";
		 if ($field == "NONE") return "No class level id was selected<br />";
         return "";
     }
	 
	 function validate_class_category_id($field)
	 {
         if ($field == "") return "No class category was entered<br />";
		 if ($field == "NONE") return "No class category id was selected<br />";
         return "";
     }
