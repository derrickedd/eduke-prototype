...
<body>
     <div id="frame">
	     ...
	     <div id="navigation">
		     ...
		     <p class="class-nav">
			     ...
			     <span class="extra-class-nav">
				     ...
				     <a href="../class/classlevel.php" class="selected-nav">Class Level</a>
				 </span>
	     ...
	     <div id="content-reload">
			 <div id="boxlist-content">
				 ...
				 <div id="main">
				     <?php 
					    echo "<div id=\"list-classlevel\">";
						
					 echo "<p style=\"padding:10px;background-color:#456784;color:white;\">
							 <span style=\"width:50px;\"><b>ID</b></span>
							 <span style=\"width:170px;\"><b>CLASS LEVEL</b></span>
							 <span style=\"width:200px;\"><b>REPORT TYPE</b></span>
						 </p>";
						 
						 $query = "SELECT * FROM classlevel";
						 $result = mysql_query($query);
						 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
						 ...
						 echo "<ul>";
							 while($row=mysql_fetch_array($result)){
								 ...
								 $classlevel_name = $row['classlevel_name'];
								 $report_type = $row['report_type'];
								 
								 echo "<li>
									 <span style=\"width:50px;\">$classlevel_id</span>
									 <span style=\"width:170px;\">$classlevel_name</span>
									 <span style=\"width:170px;\">$report_type</span>
									 <input type=\"button\" value=\"Delete\" style=\"padding:5px;width:80px;background-color:black;
										 border:solid 1px black;border-radius:3px;color:red;cursor:pointer;\" onclick=deleteClassLevel($classlevel_id)>
								  </li>";
							 }
						 echo "</ul>";
					echo "</div>";
					 ?>
					 <div id="addclasscategory">
					     <h3>CREATE CLASS LEVEL</h3>
					     <p><b>CLASS LEVEL</b> <br><input type="text" id="classlevel_name" placeholder="Class Level. E.g. A-Level"></p>
					     <p><b>REPORT TYPE</b> <br><select type="text" id="report_type">
						     <option value="Type A">Type A</option>
						     <option value="Type B">Type B</option>
						     <option value="Type C">Type C</option>
						 </select></p>
					     <p><input type="button" value="Create Class Level" style="background-color:green;border:solid 1px green;cursor:pointer;color:#fff;
						 border-radius:3px;" onclick=addClassLevelData()></p>
					 </div>
				 </div>
	 ...
	 <script>
		 function deleteClassLevel(classlevel_id){
			 if (confirm('Are you sure you want to delete the class level?')) {
				 $.ajax({
					 type:"post",
					 url:"class-process-function.php",
					 data:"classlevel_id="+classlevel_id+"&action=deleteclasslevel",
					 success:function(data){
						 window.location.href = "classlevel.php";
					 }
				 });
			 }
		 }
		 <?php
		     echo "function addClassLevelData(){
			 var classlevel_name = $(\"#classlevel_name\").val();
			 ...
			 if (classlevel_name == ''){
				 ...
				 $.ajax({
					 type:\"post\",
					 url:\"class-process-function.php\",
					 data:\"classlevel_name=\"+classlevel_name+\"&report_type=\"+report_type+\"&action=addclassleveldata\",
					 success:function(data){
						 location.reload();
					 }
				 });
