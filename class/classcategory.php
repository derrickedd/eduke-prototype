...
<html lang="en">
<head>
     <title>Eduke - Class Category</title>
...
<body>
	     ...
	     <div id="navigation">
		     ...
		     <p class="class-nav">
			     ...
			     <span class="extra-class-nav">
				     ...
				     <a href="../class/classcategory.php" class="selected-nav">Class Category</a>
			 ...
			 <div id="boxlist-content">
				 ...
				 <div id="main">
					 ...
					 echo "<p style=\"padding:10px;background-color:#456784;color:white;\">
							 ...
							 <span style=\"width:200px;\"><b>CLASS CATEGORY</b></span>
							 
						 </p>";
						 
						 $query = "SELECT * FROM classcategory";
						 $result = mysql_query($query);
						 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
						 ...
						 echo "<ul>";
							 while($row=mysql_fetch_array($result)){
								 ...
								 echo "<li>
									 <span style=\"width:50px;\">$class_category_id</span>
									 <span style=\"width:200px;\">$class_category</span>
									 <input type=\"button\" value=\"Delete\" style=\"padding:5px;width:80px;background-color:black;
										 border:solid 1px black;border-radius:3px;color:red;cursor:pointer;\" onclick=deleteClassCategory($class_category_id)>
								  </li>";
							 }
						 echo "</ul>";
					echo "</div>";
					 ...
					 <div id="addclasscategory">
					     <h3>CREATE CLASS CATEGORY</h3>
					     <p><b>CLASS CATEGORY</b> <br><input type="text" id="class_category" placeholder="Class Category. E.g. Arts"></p>
					     <p><input type="button" value="Create Class Category" style="background-color:green;border:solid 1px green;cursor:pointer;color:#fff;
						 border-radius:3px;" onclick=addClassCategoryData()></p>
					 </div>
				 </div>
	 ...
	 <script>
		 function deleteClassCategory(class_category_id){
			 if (confirm('Are you sure you want to delete the class category?')) {
				 $.ajax({
					 type:"post",
					 url:"class-process-function.php",
					 data:"class_category_id="+class_category_id+"&action=deleteclasscategory",
					 success:function(data){
						 window.location.href = "classcategory.php";
					 }
				 });
			 }
		 }
		 <?php
		     echo "function addClassCategoryData(){
			 ...
			 if (class_category == ''){
				 ...
				 $.ajax({
					 type:\"post\",
					 url:\"class-process-function.php\",
					 data:\"class_category=\"+class_category+\"&action=addclasscategorydata\",
					 success:function(data){
						 location.reload();
					 }
				 });
