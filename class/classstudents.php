...
<body>
	 ...
	 <script>
	 <?php
	     echo "function showClassStudents(class_id){
			 $.ajax({
				 type:\"post\",
				 url:\"class-process-function.php\",
				 data:\"class_id=\"+class_id+\"&action=showclassstudents\",
				 success:function(data){
					 $(\"#list-results\").html(data);
				 }
			 });
		 }
         showClassStudents($class_id);";
	 ?>
	     
		 
		 function moreClassStudents(value, class_id){
			 $.ajax({
				 type:"post",
				 url:"class-process-function.php",
				 data:"page="+value+"&class_id="+class_id+"&action=showclassstudents",
				 success:function(data){
					 $("#list-results").html(data);
				 }
			 });
		 ...
		 function promoteClassStudent(student_id,class_id, id1, id2){
			 var term_id = $("#"+id2).val();
			 var sub_class_id = $("#"+id1).val();alert();
