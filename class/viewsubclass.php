     ...
     <div id="frame">
	     ...
	     <div id="content-reload">
			 <div id="boxlist-content">
				 ...
				 <div id="main">
				     <?php
					     if(isset($_GET['sub_class_id']) && isset($_GET['class_id']) )
					     {
							 $sub_class_id = $_GET['sub_class_id'];
							 $class_id = $_GET['class_id'];
							 
							 $query = "SELECT * FROM schoolsubclass WHERE sub_class_id = $sub_class_id";
							 $result = mysql_query($query);
							 if(!$result) die("DB access failed(derro internal error): " . mysql_error());
							 $rows = mysql_num_rows($result);
							 ...
							 for ($j = 0 ; $j < $rows ; ++$j)
							 {
								 $row = mysql_fetch_row($result);
								 $sub_class_id = $row[0];
								 $sub_class_alias = $row[1];
								 $class_category_id = $row[3];
								 $teacher_id = $row[4];
								 
								 
								 echo "<div id=\"view-class\">
									 <h3>EDIT CLASS STREAM</h3>
									 <div class=\"grid-left\">";
									     echo "<p><b>CLASS NAME</b> <br><input type=\"text\" id=\"class_alias\" value=\"$row[1]\"></p>";
										 
										 $queryex = "SELECT teacher_id, teacher_firstname,teacher_lastname FROM schoolteachers";
										 ...
										 echo "<p><b>CLASS TEACHER</b> <br> <select id=\"teacher_id\">";
										 for ($jex = 0 ; $jex < $rowsex ; ++$jex)
										 {
											 ...
											 for ($kex = 0 ; $kex < 1 ; ++$kex)
												 echo "<option ";
												 if ($rowex[0] == $teacher_id)
													 echo " selected=\"selected\" ";
												 echo "value=$rowex[0]>$rowex[1] $rowex[2]</option>";
											 echo "</tr>";
										 }
										 echo "</select></p>";
										 
										 
										 $queryex = "SELECT class_category_id, class_category FROM classcategory";
										 $resultex = mysql_query($queryex);
										 if(!$resultex) die("DB access failed(derro internal error): " . mysql_error());
										 $rowsex = mysql_num_rows($resultex);
										 
										 echo "<p><b>CLASS CATEGORY</b> <br> <select id=\"class_category_id\">";
										 for ($jex = 0 ; $jex < $rowsex ; ++$jex)
										 {
											 $rowex = mysql_fetch_row($resultex);
											 echo "<tr>";
									 ...
									 <p>
										 <a href=\"streamstudents.php?class_id=$class_id&sub_class_id=$sub_class_id\">Stream Students</a>
										 <a href=\"classsubjects.php?class_category_id=$class_category_id\">Stream Subjects</a>
										 <a style=\"background-color:#bc3226;\" onclick=deleteSubClass($sub_class_id)>
										 Delete Class</a>
									 </p>
									 </div>";
							 }
