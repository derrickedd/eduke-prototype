...
<body>
     <div id="frame">
	     ...
	     <div id="content-reload">
			 <div id="boxlist-content">
			     <h1>PARENTS</h1>
				 <p><b>Directory: Parents</b></p>
				 
				 <div id="buttons">
				     <input type="text" placeholder="Search Parent" id="search_parent" style="float:none;">
				 </div>
	 ...
	 <script>
	     ...
	     var $input3 = $('#search_parent');

		 /*on keyup, start the countdown*/
		 $input3.on('keyup', function () {
			 clearTimeout(typingTimer);
			 typingTimer = setTimeout(doneParentTyping, doneTypingInterval);
		 });
		 ...
		 function doneParentTyping () {
			 var search_parent = $("#search_parent").val();
			 
			 $.ajax({
				 type:"post",
				 url:"parent-process-function.php",
				 data:"search_parent="+search_parent+"&action=showparentlistaftersearch",
		 ...
		 function moreParents(value){
			 $.ajax({
				 type:"post",
				 url:"parent-process-function.php",
				 data:"page="+value+"&action=showparentlist",
				 success:function(data){
					 $("#list-results").html(data);
				 }
			 });
		  ...
		  function moreSearchParents(value){
			 var search_parent = $("#search_parent").val();
			 
			 $.ajax({
				 type:"post",
				 url:"parent-process-function.php",
				 data:"page="+value+"&search_parent="+search_parent+"&action=showparentlistaftersearch",
				 success:function(data){
					 $("#list-results").html(data);
				 }
			 });
