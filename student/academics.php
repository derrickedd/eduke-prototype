...
<html lang="en">
<head>
     <title>Eduke - Student Academics</title>
...
<body>
     <div id="frame">
	     ...
	     <div id="content-reload">
			 <div id="boxlist-content">
				 ...
				 <p><img src="../../system/images/sys-nav/house-red.png" height="15px"> <b>Students</b> / <b>Details</b> / <b>Subjects & Marks</b></p>
				 
				 <div id="main">
				 <?php
				         
					 if(isset($_GET['student_id']) && isset($_GET['class_id']) && isset($_GET['admission_academic_term_id']) 
						 && isset($_GET['sub_class_id']))
				     {
						 $student_id = $_GET['student_id'];
						 $class_id = $_GET['class_id'];
						 $sub_class_id = $_GET['sub_class_id'];
						 $admission_academic_term_id = $_GET['admission_academic_term_id'];
						 ...
						 /*Looking for terms student has done*/
						 $queryq = "SELECT academic_term_id FROM classenrolled WHERE class_id = '$class_id' AND student_id = '$student_id'";
						 $resultq = mysql_query($queryq);
						 if(!$resultq) die("DB access failed(derro internal error): " . mysql_error());
						 $rowsq = mysql_num_rows($resultq);
						 
						 echo "<div id=\"selecting-pan\">";
						     echo "<div id=\"term-selection-pan\">";
								 ...
								 for ($jq = 0 ; $jq < $rowsq ; ++$jq)
								 {
									 ...
									 $term_id_ref = $rowq[0]; /*variable for button display*/
											 
									 $queryex = "SELECT academic_term_id, academic_year, academic_term FROM schoolterm WHERE academic_term_id = '$term_id_ref' 
									 ...
									 for ($jex = 0 ; $jex < $rowsex ; ++$jex)
									 {
										 ...
										 echo "<option value=\"$term_id\" 
												 onclick=\"checkExamsForTerm($student_id, $class_id, $sub_class_id, $term_id);
												 showReportButton($student_id, $class_id, $sub_class_id, $term_id)\">$academic_year - $academic_term</option>";
									 }
								 }
								 /*End Looking for terms student has done*/
							 echo "</select></p>";
						 echo "</div>";
						 
							 
							 echo "<div id=\"exam-selection-pan\"></div>";
							 echo "<div id=\"reportbutton\" style=\"vertical-align:middle;\"></div>";
							 
						 echo "</div>";
						 
						 echo "<div id=\"showsubjectmarks\"></div>";
	 ...
	 <div id="footer">
	  ...
	  <script>
		 function checkExamsForTerm(student_id,class_id, sub_class_id, academic_term_id){
	 
			 $.ajax({
				 type:"post",
				 url:"student-process-function.php",
				 data:"academic_term_id="+academic_term_id+"&class_id="+class_id+"&sub_class_id="+sub_class_id+"&student_id="+student_id+"&action=checkexamsforterm",
				 success:function(data){
					 $("#exam-selection-pan").html(data);
				 }
			 });
		 }
		 function showReportButton(student_id,class_id, sub_class_id, academic_term_id){
			 
			 $.ajax({
				 type:"post",
				 url:"student-process-function.php",
				 data:"academic_term_id="+academic_term_id+"&class_id="+class_id+"&sub_class_id="+sub_class_id+"&student_id="+student_id+"&action=showreportbutton",
				 success:function(data){
