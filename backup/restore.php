...
<html lang="en">
<head>
     <title>Eduke - System Restore</title>
...
<body>
     <div id="frame">
		 ...
		 <div id="content-reload">
			 <div id="boxlist-content">
				 ...
				 <p><b>Directory: Restore</b></p>
				 
				 <div id="restorewait">
				     <div class="restorewaitwhitecontent">
					     <h3>SYSTEM RESTORE</h3>
				         <p>System is being restored. Please wait...</p>
				 ...
				 if ($handle = opendir('files\.')) {
					 while (false !== ($file = readdir($handle)))
					 {
						 if ($file != "." && $file != ".." && strtolower(substr($file, strrpos($file, '.') + 1)) == 'sql')
						 {
							 ...
							 $files[filemtime($filelocation)] = $file;
						 }
					 }
					 closedir($handle);
					 ...
					 echo "<p style=\"padding:10px;background-color:#456784;color:white;\">
						 <span style=\"width:300px;\"><b>FILE NAME</b></span>
						 <span style=\"width:200px;\"><b>DATE CREATED</b></span>
					 </p>";
					 foreach($files as $file) {
						 $filelocation = "files" . '/' .$file;
						 $files[filemtime($filelocation)] = $file;
						 
						 $lastModified = date('F d Y, H:i:s',filemtime($filelocation));
						 if(strlen($file)-strpos($file,".sql")== 4){
						     if ($file == $reallyLastModified) {
							     // do stuff for the real last modified file
						     }
						     echo "<p>
							         <span style=\"width:300px\">$file</span>
									 <span style=\"width:200px\">$lastModified</span> 
									 <span style=\"width:150px\">
									     <input type=\"button\" value=\"Apply Restore\" style=\"background-color:skyblue;color:#fff;border:solid 1px skyblue;\"
										 onclick=applyRestore('$file')>
									 </span>
									 <span style=\"width:150px\">
									     <input type=\"button\" value=\"Delete Restore\" style=\"background-color:black;color:red;border:solid 1px black;\"
										 onclick=deleteRestore('$file')>
									 </span>
								 </p>";
						 }
					 }
	...
	<script>
		 <?php
		 echo "function applyRestore(file){
			 if (confirm('Are you sure you want to apply this restore?')) {
				 $('#restorewait').css('display','block');
				 $.ajax({
					 type:\"post\",
					 url:\"backup-process-function.php\",
					 data:\"file=\"+file+\"&action=applyrestore\",
		 ...
		 echo "function deleteRestore(file){
			 
			 if (confirm('Are you sure you want to delete this restore?')) {
				 $.ajax({
					 type:\"post\",
					 url:\"backup-process-function.php\",
					 data:\"file=\"+file+\"&action=deleterestore\",
					 success:function(data){
						 location.reload();
					 }
				 });
